package im.cave.ms.connection.packet.opcode;

import im.cave.ms.tools.Util;

public enum SendOpcode {
    LOGIN_STATUS(0x00),
    SERVERLIST(0x01),
    SERVERSTATUS(0x05),//CHECK_USER_LIMIT_RESULT
    CHARLIST(0x06),
    SELECT_CHARACTER_RESULT(0x07),
    AUTH_SUCCESS(0x08),
    CHECK_DUPLICATED_ID_RESULT(0x0A),
    CREATE_NEW_CHARACTER_RESULT(0x0B),
    DELETE_CHAR_TIME(0x0D),
    CANCEL_DELETE_CHAR(0x0E),
    CHANGE_CHANNEL(0x10),
    PING(0x11),
    CPING(0x12),
    CLIENT_START(0x2C),
    INIT_OPCODE_ENCRYPTION(0x2D),
    CHAR_SLOTS_EXPAND_RESULT(0x4B),
    OPEN_CREATE_CHAR(0x56),
    SERVER_LIST_BG(0x5D),
    INVENTORY_OPERATION(0x6F),
    INVENTORY_GROW(0x70),
    UPDATE_STATS(0x71),
    GIVE_BUFF(0x72),
    REMOVE_BUFF(0x73),
    CHANGE_SKILL_RESULT(0x77),
    FAME_RESPONSE(0x81),
    SHOW_STATUS_INFO(0x83),
    MAPLE_MESSAGE(0x84),
    MAP_TRANSFER_RESULT(0x85),
    GATHER_ITEM_RESULT(0x9C),
    SORT_ITEM_RESULT(0x9D),
    CHAR_INFO(0x9F),
    PARTY_RESULT(0xA0),
    RECOMMEND_PLAYER(0xA1),
    FRIEND_RESULT(0xAA),
    GUILD_RESULT(0xAD),
    SERVER_MSG(0xB0),
    CASH_PET_PICK_UP_ON_OFF_RESULT(0xB9),
    PET_AUTO_EAT_MSG(0xBA),
    EVENT_MESSAGE(0xDA),
    SERVER_NOTICE(0xD6),
    DEBUG_MSG(0xDD),
    RESULT_INSTANCE_TABLE(0xe7),
    RANK(0xF7),
    CHARACTER_POTENTIAL_SET(0x102),
    CHARACTER_POTENTIAL_RESET(0x103),
    CHARACTER_HONOR_POINT(0x104),
    CHANGE_CHAR_KEY(0x12E),
    SET_ACCOUNT_INFO(0x13C),
    UPDATE_QUEST_EX(0x13B),
    SET_AVATAR_MEGAPHONE(0x13F),
    CANCEL_TITLE_EFFECT(0x145),
    EQUIP_ENCHANT(0x16A),
    CASH_SHOP_EVENT_INFO(0x179),
    CHAR_AVATAR_CHANGE_RESULT(0x1E4),
    CHAR_AVATAR_CHANGE_SELECT(0x1E9),
    POTION_POT_MESSAGE(0x1F1),
    POTION_POT_UPDATE(0x1F3),
    CHARACTER_MODIFIED(0x1FB),
    UPDATE_VOUCHER(0x201),
    CASH_POINT_RESULT(0x219),
    SLOT_EXPAND_RESULT(0x21A),
    MACRO_SYS_DATA_INIT(0x235),
    SET_MAP(0x236),
    SET_CASH_SHOP(0x239),
    SET_CASH_SHOP_INFO(0x23A),
    WHISPER(0x242),
    FIELD_EFFECT(0x244),
    FIELD_MESSAGE(0x245), //WEDDING_MESSAGE?
    CLOCK(0x24A),
    QUICKSLOT_INIT(0x258),
    QUICK_MOVE(0x26B),
    USER_SIT(0x29B),
    USER_ENTER_FIELD(0x2B5),
    USER_LEAVE_FIELD(0x2B6),
    CHATTEXT(0x2B7),
    BLACK_BOARD(0x2B9),
    SHOW_ITEM_UPGRADE_EFFECT(0x2BC),
    SHOW_ITEM_RELEASE_EFFECT(0x2C0),
    SET_DAMAGE_SKIN(0x2DC),
    SET_SOUL_EFFECT(0x2E0),
    SIT_RESULT(0x2E1),
    FAMILIAR_OPERATION(0x2E2),
    HIDDEN_EFFECT_EQUIP(0x316),
    LIMIT_BREAK_UI(0x317),
    PET_ACTIVATED(0x318),
    PET_MOVE(0x319),
    PET_ACTION_SPEAK(0x31A),
    PET_NAME_CHANGED(0x31C),
    PET_LOAD_EXCEPTION_LIST(0x31D),
    //
    //
    //
    //
    PET_ACTION_COMMAND(0x322),
    ANDROID_CREATED(0x327),
    ANDROID_MOVE(0x328),
    ANDROID_ACTION_SET(0x329),
    ANDROID_MODIFIED(0x32A),
    ANDROID_REMOVED(0x32B),
    REMOTE_MOVE(0x345),
    REMOTE_CLOSE_RANGE_ATTACK(0x346),
    REMOTE_RANGED_ATTACK(0x347),
    REMOTE_MAGIC_ATTACK(348),
    REMOTE_HIT(0x34D),
    REMOTE_EMOTION(0x34E),
    REMOTE_SET_ACTIVE_PORTABLE_CHAIR(0x357),
    REMOTE_AVATAR_MODIFIED(0x358),
    REMOTE_EFFECT(0x359),
    UPDATE_PARTY_HP(0x35C),
    EFFECT(0x383),
    QUEST_RESULT(0x388),
    PET_SKILL_CHANGED(0x38A),
    OPEN_UI(0x390),
    CLOSE_UI(0x391),
    OPEN_UI_WITH_OPTION(0x392),
    SET_DIRECTION_MODE(0x393),
    SET_IN_GAME_DIRECTION_MODE(0x394),
    SET_STAND_ALONE_MODE(0x395),
    NOTICE_MSG(0x3A2),
    CHAT_MSG(0x3A3),
    IN_GAME_DIRECTION_EVENT(0x3B2),
    SET_DEAD(0x3CB),
    OPEN_DEAD_UI(0x3CC),
    FINAL_ATTACK(0x3D5),
    ILLUSTRATION_MSG(0x405),
    SKILL_COOLTIME_SET(0x40F),
    DAMAGE_SKIN_SAVE_RESULT(0x42D),
    OPEN_WORLDMAP(0x48F),
    SPAWN_SUMMON(0x4B7),
    SUMMON_MOVE(0x4B9),
    SPAWN_MOB(0x4D5),
    REMOVE_MOB(0x4D6),
    MOB_CHANGE_CONTROLLER(0x4D7),
    MOB_MOVE(0x4DE),
    MOB_CONTROL_ACK(0x4DF),
    HP_INDICATOR(0x4EC),
    SPAWN_NPC(0x542),
    REMOVE_NPC(0x543),
    SPAWN_NPC_REQUEST_CONTROLLER(0x545),
    NPC_ANIMATION(0x549),
    DROP_ENTER_FIELD(0x569),
    PICK_UP_DROP(0x56B),
    NPC_TALK(0x741),
    NPC_SHOP_OPEN(0x742),
    NPC_SHOP_RESULT(0x743),
    TRUNK_OPERATION(0x75A),
    CHAT_ROOM(0x760),
    TRADE_ROOM(0x761),
    PARCEL_DLG(0x7BC),
    CASH_SHOP_QUERY_CASH_RESULT(0X7BE),
    CASH_SHOP_CASH_ITEM_RESULT(0x7BF),
    POTION_POT_CREATE(0x7D3),
    SIGNIN_REWARDS(0x7EF),
    OPEN_UNITY_PORTAL(0x7F3),
    KEYMAP_INIT(0x84A),
    BATTLE_ANALYSIS(0x879),
    REMAINING_MAP_TRANSFER_COUPON(0x897),
    ;
    private final int code;

    SendOpcode(int code) {
        this.code = code;
    }

    public short getValue() {
        return (short) code;
    }

    public static Object getByValue(short op) {
        return Util.findWithPred(values(), sendOpcode -> sendOpcode.code == op);
    }

}
